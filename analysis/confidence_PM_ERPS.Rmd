...
d1 <- read_csv("L:/alan/confidence_2019/analüüs/OSF/data/31-Oct-2020-ERN_PE_export.csv")
  
# P3 dataset
d2 <- read_csv("L:/alan/confidence_2019/analüüs/OSF/data/31-Oct-2020-P3export.csv")
...
# mixed model forconfidence


fitc = lmer(Mean_confidence ~ difficulty*fluency + (1|participant), data=table_sum)
anova(fitc)
...
#plotd = d %>% group_by(participant, difficulty, fluency) %>%
#summarise("Mean pooled ERN"= mean(ERN70_60_Fz, na.tm=T),
#"Mean pooled Pe"= mean(Pe200_300_Cz, na.tm=T))
...
```{r}
options(scipen=9999)
# ern & confidence (0.06, significant)
...
# P3 & confidence (-0.005, nonsignificant)
...
# correct trials and ERN (biggest correlation - 0.07, significant)
...
# summary
mlm_summary(fit1)                              # dependent variable
...
```{r}
# difficulty & ERN mediation model for error trials
mediffERN <- isolate(d = d2,
        by = "participant",   # level variable
        value = c("ERN","confidence"))  


mediffERN$difficulty = as.numeric(as.factor(mediffERN$difficulty))
# select only error responses

ERN_error = mediffERN %>% filter(response == "error")

                             
                               
# fit difficulty mediation model

fitERN <- mlm(d=ERN_error,
           id = "participant",          # subject variable
           x = "difficulty",            # experimental condition
           m = "ERN_cw",                  # mediator
           y = "confidence_cw",
...
# summary
mlm_summary(fitERN)                              # dependent variable

# path plot
mlm_path_plot(fitERN, xlab = "Difficulty", mlab = "ERN", ylab = "Confidence")
...
ERN_correct = mediffERN %>% filter(response == "correct")

                              
                               
# fit difficulty mediation model

fitERN2 <- mlm(d=ERN_correct,
           id = "participant",          # subject variable
           x = "difficulty",            # experimental condition
           m = "ERN_cw",                  # mediator
           y = "confidence_cw",
...
# summary
mlm_summary(fitERN2)                              # dependent variable

# path plot
mlm_path_plot(fitERN2, xlab = "Difficulty", mlab = "ERN", ylab = "Confidence")
...
# fluency & ERN mediation model for error trials
mefluERN <- isolate(d = d2,
        by = "participant",   # level variable
        value = c("confidence","ERN")) # siia ei saa conf panna, kuna siis p3 läheb NAks. 


mefluERN$fluency = as.numeric(as.factor(mefluERN$fluency))
# select only error responses

flu_error = mefluERN %>% filter(response == "error")
...
# path plot
mlm_path_plot(fit_flu_error, xlab = "Fluency", mlab = "ERN", ylab = "Confidence")


# correct, fluency ERN

flu_correct = mefluERN %>% filter(response == "correct")
...
# path plot
mlm_path_plot(fit_flu_correct, xlab = "Fluency", mlab = "ERN", ylab = "Confidence")
